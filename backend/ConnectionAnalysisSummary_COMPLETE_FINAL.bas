Attribute VB_Name = "ConnectionAnalysisSummary"
Option Explicit
...
NextCell:
    ...
    Dim raw() As Variant
    ReDim raw(0 To nPlanets - 1, 0 To 13)
    ...
    For p = 0 To nPlanets - 1
        ...
        ' ================================
        ' MD-anchored continuous connectivity (ALL SIX LEVELS)
        ' ================================
        Dim lc As Long
        lc = 0
        ...
        ' ================================
        ' LevelsConnected_NOSSL
        ' Same 6-level connectivity
        ' Ignore rows SSL and NLofSSL ONLY
        ' ================================
        Dim lcNoSSL As Long
        If planets(p) = "SSL" Or planets(p) = "NLofSSL" Then
            lcNoSSL = 0
        Else
            lcNoSSL = lc
        End If
        ...
        raw(p, 8) = lcNoSSL
        raw(p, 9) = freqTotal(p)
        raw(p, 10) = freqNLType(p)
        raw(p, 11) = Score_SCORE(raw(p, 0))
        raw(p, 12) = Score_WTDSCORE(raw(p, 0))
        raw(p, 13) = Score_WTDSCORE_NOSSL(raw(p, 0))
    Next p
    ...
    Dim out() As Variant
    ReDim out(0 To nPlanets, 0 To 12)

    Dim header As Variant
    header = Array("Planet", "MD", "AD", "PD", "SU", "PRANA", "DEH", _
                   "LevelsConnected","LevelsConnected_NOSSL", "Freq_Total", "Freq_NLType", _
                   "Score", "WTDSCORE", "WTDSCORE_NOSSL")
...
Public Sub BatchAnalyze_MD_Combinations_Scaled()
    ...
    ' === FILTER CONFIG ===
    Const MIN_LEVELS_CONNECTED As Long = 4      ' focus only on Levels 56
    ...
    For R = 2 To lastRow
        ...
        If Trim$(CStr(wsComb.Cells(R, cMD).Value)) <> "" Then
            ...
            ' === TRY ALL 9×9×9 combinations for SUPRDEH ===
                ...
                For iPR = LBound(planets) To UBound(planets)
                    ...
                    For iDEH = LBound(planets) To UBound(planets)
                        ...
                        For i = 1 To UBound(arrSummary, 1)
                            ...
                            ' Filter: focus only on LevelsConnected 56
