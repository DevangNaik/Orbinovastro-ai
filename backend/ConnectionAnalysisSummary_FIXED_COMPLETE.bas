...
' === Helper: fetch column index with alias/normalization ===
Private Function CAS_NormColName(ByVal s As String) As String
    s = UCase$(s)
    s = Replace(s, " ", "")
    s = Replace(s, "#", "")
    s = Replace(s, ".", "")
...
Private Function CAS_GetColIndex(ByVal dict As Object, ParamArray names() As Variant) As Long
    ...
    For i = LBound(names) To UBound(names)
        ...
        If dict.Exists(key) Then
            CAS_GetColIndex = CLng(dict(key))
            Exit Function
        End If
        key = CAS_NormColName(key)
        If dict.Exists(key) Then
            CAS_GetColIndex = CLng(dict(key))
            Exit Function
        End If
...
Private Function ConnSummaryCore(ByVal ws As Worksheet, _
    ...
    Dim dataArr As Variant: dataArr = dataR.Value
    ...
    Dim s As String, t As String, key As String
    Dim tokens As Variant

    For r = 1 To UBound(dataArr, 1)
        For c = 1 To UBound(dataArr, 2)

            s = Replace(CStr(dataArr(r, c)), " ", "")
...
NextCell:
    ...
    ' ---- OUTPUT (expanded to 19 columns) ----
    Dim raw() As Variant
    ReDim raw(0 To nPlanets - 1, 0 To 19)

    Dim lc As Long, win As Variant
    ...
    For p = 0 To nPlanets - 1
        ...
        If raw(p, 1) = 1 Then
            lc = 1
            For c = 2 To 6
                If raw(p, c) = 1 Then lc = lc + 1 Else Exit For
            Next c
        End If
        ...
        win = CAS_GetDasaWindow_Internal(lc, _
              ws.Range("GG20").Value, ws.Range("GH20").Value, ws.Range("GI20").Value, _
              ws.Range("GJ20").Value, ws.Range("GK20").Value, ws.Range("GL20").Value)
...
' ============================================================
' === VimDasaTbl CACHE (MERGED FROM Module2.bas) ===
' ============================================================



' ============================================================
'  VIMDASA CACHE + WINDOW LOOKUP (FINAL, FIXED)
' ============================================================
...
Public Sub CAS_LoadVimDasaCache()
    ...
    ' Find VimDasaTbl anywhere in this workbook
    For Each ws In ThisWorkbook.Worksheets
        On Error Resume Next
    ...
    If lo Is Nothing Then
        Err.Raise 5, , "VimDasaTbl not found in this workbook"
    End If

    If lo.DataBodyRange Is Nothing Then
        Err.Raise 5, , "VimDasaTbl has no data rows"
    End If
    ...
    For Each lc In lo.ListColumns
        ...
        ' store normalized name (ROWNO, DASADATE, etc.)
        norm = CAS_NormColName(lc.Name)
        If Not VimDasaCol.Exists(norm) Then VimDasaCol(norm) = lc.Index
    Next lc
...
'Public 
Function CAS_GetDasaWindowCachedFromRange( _
    ...
    CAS_GetDasaWindowCachedFromRange = CAS_GetDasaWindow_Internal(LevelsConnected, md, ad, pd, su, prana, deh)
End Function
...
Public Function CAS_GetDasaWindowCachedFromRange(ByVal LevelsConnected As Long) As Variant
    ...
    CAS_GetDasaWindowCachedFromRange = CAS_GetDasaWindow_Internal( _
        LevelsConnected, _
        ws.Range("GG20").Value, _
        ws.Range("GH20").Value, _
        ws.Range("GI20").Value, _
        ws.Range("GJ20").Value, _
        ws.Range("GK20").Value, _
        ws.Range("GL20").Value _
    )
End Function
...
Public 
Function CAS_GetDasaWindow_Internal( _
    ByVal LevelsConnected As Long, _
    ...
    If Not VimDasaLoaded Then CAS_LoadVimDasaCache

    Dim idxMD As Long, idxAD As Long, idxPD As Long
    Dim idxSU As Long, idxPR As Long, idxDEH As Long
    Dim idxDate As Long, idxRowNo As Long

    idxMD = CAS_GetColIndex(VimDasaCol, "MD")
    idxAD = CAS_GetColIndex(VimDasaCol, "AD")
    idxPD = CAS_GetColIndex(VimDasaCol, "PD")
    ' SU is often labeled SD or Sukshma in your VimDasaTbl
    idxSU = CAS_GetColIndex(VimDasaCol, "SU", "SD", "SUKSHMA")
    idxPR = CAS_GetColIndex(VimDasaCol, "PRANA")
    ' DEH is often labeled DEHA
    idxDEH = CAS_GetColIndex(VimDasaCol, "DEH", "DEHA", "DEH", "DEHA7")
    idxDate = CAS_GetColIndex(VimDasaCol, "DasaDate", "DASADATE")
    idxRowNo = CAS_GetColIndex(VimDasaCol, "RowNo", "ROWNO", "ROW", "ROW#", "ROWNUMBER")

    If idxMD = 0 Or idxAD = 0 Or idxPD = 0 Or idxSU = 0 Or idxPR = 0 Or idxDEH = 0 Or idxDate = 0 Then
    ...
    ' Find the first matching row (prefix match based on LevelsConnected),
    ' then extend until the prefix changes (contiguous block).
    For r = 1 To VimDasaRowCount
        If (LevelsConnected < 1 Or VimDasaArr(r, idxMD) = md) _
        And (LevelsConnected < 2 Or VimDasaArr(r, idxAD) = ad) _
        And (LevelsConnected < 3 Or VimDasaArr(r, idxPD) = pd) _
        And (LevelsConnected < 4 Or VimDasaArr(r, idxSU) = su) _
        And (LevelsConnected < 5 Or VimDasaArr(r, idxPR) = prana) _
        And (LevelsConnected < 6 Or VimDasaArr(r, idxDEH) = deh) Then
    ...
    dStart = CDate(VimDasaArr(rStart, idxDate))

    If rEnd < VimDasaRowCount Then
        dEnd = CDate(VimDasaArr(rEnd + 1, idxDate))
    Else
        dEnd = dStart
    End If

    durDays = CDbl(dEnd) - CDbl(dStart)

    Dim outStartRow As Long, outEndRow As Long
    If idxRowNo > 0 Then
        outStartRow = CLng(VimDasaArr(rStart, idxRowNo))
        outEndRow = CLng(VimDasaArr(rEnd, idxRowNo))
    Else
        outStartRow = rStart
        outEndRow = rEnd
    End If

    CAS_GetDasaWindow_Internal = Array( _
        outStartRow, _
        outEndRow, _
        dStart, _
        dEnd, _
...
Public Function FixTiny(ByVal v As Variant, Optional ByVal roundTo2 As Boolean = False) As Variant
    ...
    If IsNumeric(v) Then
        ...
        ' eliminate floating noise
        If Abs(x) < 0.0000001 Then
            FixTiny = 0#
        ElseIf roundTo2 Then
            FixTiny = Round(x, 2)
        Else
            FixTiny = x
        End If
