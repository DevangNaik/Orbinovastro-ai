...
NextCell:
    ...
    ' --- Read current MD..DEH selection from the sheet (GG20:GL20) ---
    Dim md As String, ad As String, pd As String, su As String, prana As String, deh As String
    On Error Resume Next
    md = CStr(ws.Range("GG20").Value)
    ad = CStr(ws.Range("GH20").Value)
    pd = CStr(ws.Range("GI20").Value)
    su = CStr(ws.Range("GJ20").Value)
    prana = CStr(ws.Range("GK20").Value)
    deh = CStr(ws.Range("GL20").Value)
    On Error GoTo fail
    ...
    For p = 0 To nPlanets - 1
        ...
        ' --- Window (start/end) from VimDasaTbl based on LevelsConnected ---
        Dim win As Variant
        win = CAS_GetDasaWindowFromLevels(lc, md, ad, pd, su, prana, deh)

        ' Columns 13..19: StartRow, EndRow, StartDate, EndDate, DurDays, DurYears, DurMonths
        If IsArray(win) Then
            raw(p, 13) = win(0)
            raw(p, 14) = win(1)
            raw(p, 15) = win(2)
...
' =============================================================================
'  DASA WINDOW HELPERS (added)
'  Purpose:
'    Return StartRow/EndRow/StartDate/EndDate and durations for the current
'    MD..DEHA combination, using only the first N levels (N = LevelsConnected).
'
'  IMPORTANT COLUMN NAME MAPPING (VimDasaTbl):
'    MD    -> "MD"
'    AD    -> "AD"
'    PD    -> "PD"
'    SU    -> "SD"   (Sukshma column in your table)
'    PRANA -> "PRANA"
'    DEH   -> "DEHA" (Deha column in your table)
'    Date  -> "DasaDate"
'    Row#  -> tries: "row no.", "RowNo", "Row #", "RowNum" (optional)
' =============================================================================

Private CAS_VimArr As Variant
Private CAS_VimRowCount As Long
Private CAS_VimCol As Object
Private CAS_VimLoaded As Boolean

' ---- find a ListObject by name anywhere in ThisWorkbook ----
Private Function CAS_FindTableAcrossWorkbook(ByVal tableName As String) As ListObject
    Dim ws As Worksheet
    Dim lo As ListObject
    For Each ws In ThisWorkbook.Worksheets
...
' ---- load VimDasaTbl into memory once ----
Public Sub CAS_LoadVimDasaCache()
    Dim lo As ListObject
    Dim lc As ListColumn

    If CAS_VimLoaded Then Exit Sub

    Set lo = CAS_FindTableAcrossWorkbook("VimDasaTbl")
    If lo Is Nothing Then Err.Raise 5, , "VimDasaTbl not found (any sheet)"

    If lo.DataBodyRange Is Nothing Then Err.Raise 5, , "VimDasaTbl has no rows"

    CAS_VimArr = lo.DataBodyRange.Value
    CAS_VimRowCount = UBound(CAS_VimArr, 1)

    Set CAS_VimCol = CreateObject("Scripting.Dictionary")
    CAS_VimCol.CompareMode = vbTextCompare

    For Each lc In lo.ListColumns
        CAS_VimCol(lc.Name) = lc.Index
    Next lc
...
' ---- safe column lookup ----
Private Function CAS_ColIdx(ByVal colName As String) As Long
    If CAS_VimCol Is Nothing Then CAS_ColIdx = 0: Exit Function
    If CAS_VimCol.Exists(colName) Then
        CAS_ColIdx = CLng(CAS_VimCol(colName))
    Else
        CAS_ColIdx = 0
    End If
End Function

' ---- try multiple possible names for row-number column ----
Private Function CAS_RowNoIdx() As Long
    Dim nm As Variant
    For Each nm In Array("row no.", "RowNo", "Row #", "RowNum", "Row")
        CAS_RowNoIdx = CAS_ColIdx(CStr(nm))
        If CAS_RowNoIdx <> 0 Then Exit Function
    Next nm
    CAS_RowNoIdx = 0
End Function

' ---- main: compute window for LevelsConnected using provided MD..DEH values ----
Public Function CAS_GetDasaWindowFromLevels( _
    ...
    If LevelsConnected <= 0 Then
        CAS_GetDasaWindowFromLevels = Array(0, 0, 0, 0, 0#, 0#, 0#)
        Exit Function
    End If

    If Not CAS_VimLoaded Then CAS_LoadVimDasaCache

    Dim iMD As Long, iAD As Long, iPD As Long, iSD As Long, iPR As Long, iDEH As Long, iDate As Long
    Dim iRowNo As Long
    iMD = CAS_ColIdx("MD")
    iAD = CAS_ColIdx("AD")
    iPD = CAS_ColIdx("PD")
    iSD = CAS_ColIdx("SD")          ' Sukshma column (used as SU)
    iPR = CAS_ColIdx("PRANA")
    iDEH = CAS_ColIdx("DEHA")       ' Deha column
    iDate = CAS_ColIdx("DasaDate")
    iRowNo = CAS_RowNoIdx()

    If iMD = 0 Or iAD = 0 Or iPD = 0 Or iSD = 0 Or iPR = 0 Or iDEH = 0 Or iDate = 0 Then
        Err.Raise 5, , "VimDasaTbl missing required columns (MD/AD/PD/SD/PRANA/DEHA/DasaDate)"
    End If

    Dim useMD As Boolean, useAD As Boolean, usePD As Boolean, useSU As Boolean, usePR As Boolean, useDEH As Boolean
    ...
    For r = 1 To CAS_VimRowCount
        If (Not useMD Or StrComp(CStr(CAS_VimArr(r, iMD)), md, vbTextCompare) = 0) _
        And (Not useAD Or StrComp(CStr(CAS_VimArr(r, iAD)), ad, vbTextCompare) = 0) _
        And (Not usePD Or StrComp(CStr(CAS_VimArr(r, iPD)), pd, vbTextCompare) = 0) _
        And (Not useSU Or StrComp(CStr(CAS_VimArr(r, iSD)), su, vbTextCompare) = 0) _
        And (Not usePR Or StrComp(CStr(CAS_VimArr(r, iPR)), prana, vbTextCompare) = 0) _
        And (Not useDEH Or StrComp(CStr(CAS_VimArr(r, iDEH)), deh, vbTextCompare) = 0) Then

            If rStart = 0 Then rStart = r
            rEnd = r
        Else
            ' once we started a matching block, stop at first mismatch
            If rStart <> 0 Then Exit For
        End If
    Next r
    ...
    Dim startRowNo As Long, endRowNo As Long
    If iRowNo <> 0 Then
        startRowNo = CLng(Val(CAS_VimArr(rStart, iRowNo)))
        endRowNo = CLng(Val(CAS_VimArr(rEnd, iRowNo)))
        If startRowNo = 0 Then startRowNo = rStart
        If endRowNo = 0 Then endRowNo = rEnd
    Else
        startRowNo = rStart
        endRowNo = rEnd
    End If
    ...
    dStart = CAS_VimArr(rStart, iDate)

    If rEnd < CAS_VimRowCount Then
        dEnd = CAS_VimArr(rEnd + 1, iDate)
    Else
        dEnd = dStart
    End If
    ...
    If IsDate(dStart) And IsDate(dEnd) Then
        durDays = CDbl(CDate(dEnd) - CDate(dStart))
    Else
        durDays = 0#
    End If
    ...
    CAS_GetDasaWindowFromLevels = Array( _
        startRowNo, _
        endRowNo, _
        dStart, _
        dEnd, _
